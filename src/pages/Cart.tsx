...
const Cart: React.FC = () => {
  ...
  const groupedItems = state.items.reduce(
    (acc: { [key: number]: CartItem }, item: CartItem) => {
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className={`flex items-center ${textColor}`}>
                      ...
                      <QuantitySelector
                        quantity={
                          editedQuantities[item.product.id] || item.quantity
                        }
                    ...
                    <p className={textColor}>
                      Valor Unitário: R$ {item.product.price.toFixed(2)}
                    </p>
                    <p className={textColor}>
                      Valor Total: R$ {getTotalPrice(item)}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => openModal(item.product)}
                  className={`bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 hover:text-white`}
                >
                  <FaTrash />
                </button>
              </li>
